...
The first fresh mission on A.22/A.23-loaded code completed the full readonly discovery workflow through `project_report`, `role_pipeline_run` and `compose_result`, then correctly blocked cross-TaskRun continuation at the new frontier `SEMANTIC_REASONER_ROLE_BUDGET_EXCEEDED`; no child TaskRun or new write authority was materialized. The next M10 diagnostic target is therefore continuation semantic-reasoner budget, not either intra-workflow safety bridge.
