...
The next isolated M10 frontier is M10-A.27: `MissionCompletionBindingProposalService` names `mission_completion_evidence_binder`, but that role has no RoleRegistry definition, model binding, alias-compatible router identity, or role output contract. A synthetic M9-shaped mission with real reasoner returns `SEMANTIC_REASONER_ROLE_BINDING_UNAVAILABLE`; the current fresh FireTest does not activate this branch because its mission snapshot contains zero completion and validation requirements. A simple alias is not a safe repair: the default Qwen3 1.7B route rejects the binder role, while the evidence catalog is unbounded for cognition (catalog build limit 1000); measured binder prompt size crosses the low-role 6k budget at 10 synthetic evidence rows and the medium 12k budget at 30. A.27 must therefore define a coherent binder role/model/output-budget contract together with bounded evidence projection or batching, rather than merely renaming the caller.
