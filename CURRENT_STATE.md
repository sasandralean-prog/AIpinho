...
Immediate frontier: continue Sprint M10 from `e2enewruntimee.md`. M10-A.17 implementation/main SHA `0a803b6d6b36c95065d11b2b70f96992439e089e` closes the semantic-reasoner robustness frontier exposed after A.16 without adding a second runtime or authority. The persisted replay showed that authoritative requirement provenance dominated the model payload, the semantic role budget was marked exceeded but not enforced, llama.cpp context sizing underestimated the tokenizer, context-window overflow was misclassified through JSON validation, and retry could grow an already-overflowing prompt. The canonical fix now gives the model a bounded semantic requirements view while deterministic provenance remains outside model authority, fails closed when the role budget is exceeded, sizes context conservatively/configurably, records context overflow explicitly, retries overflow without prompt growth, and validates nested governed `json_shape` through the existing OutputContract/Evaluator/RetryPolicy chain. Real read-only replay now reaches accepted limitation compatibility and `continue_to_next_phase` with no authority gap; dependency admission is `ADMITTED_WITH_CONSTRAINTS` for the existing next phase candidate. The canonical backend was then restarted through `BackendControlService`/canonical scripts with restart id `backend_restart_1789955928500`; pre- and post-health were both `healthy`, `/api/v1/health`, `/api/v1/runtime/health`, TaskRuntime and role-inference status all returned `ok`, and pre-existing dirty tracked/untracked workspace state was preserved. The next M10 activity can therefore use the freshly restarted live backend for manual E2E observation. Mission-level completion truth remains evidence-bound across canonical TaskRuns and subordinate to RuntimeTruth/SpeakerTruth. The deferred Windows subprocess cp1252/.m4a reader issue remains separate unless a later mission explicitly scopes it.
