...
- Canonical backend restart used `BackendControlService` and the configured `scripts/dev/stop_aipinho_9088.ps1` / `start_aipinho_9088.ps1` scripts. Restart id `backend_restart_1789955928500` returned `accepted` with no warnings and both pre/post health `healthy`; external checks confirmed `/api/v1/health=ok`, `/api/v1/runtime/health=ok`, TaskRuntime `status=ok`/`mode=governed_controlled`, role inference `status=ok`, and canonical Git `HEAD == origin/main == 0a803b6d6b36c95065d11b2b70f96992439e089e` at runtime restart validation.
