...
- **M10-A.27 diagnostic frontier - mission completion evidence binder role:** `MissionCompletionBindingProposalService` has referenced `role_id=mission_completion_evidence_binder` since original M9 commit `84df6a48`, but no matching role exists in `default_roles.yaml`, `role_model_bindings.yaml`, role aliases, or role output contracts. A real synthetic M9-shaped snapshot/catalog with requirements returns `SEMANTIC_REASONER_ROLE_BINDING_UNAVAILABLE`. The current fresh FireTest snapshot has no completion/validation requirements, so its proposer exits through `MISSION_COMPLETION_REQUIREMENTS_ABSENT` and does not activate this bug. A role alias alone is insufficient: the default Qwen3 1.7B route rejects the unknown binder role with `model_capability_or_role_mismatch`; Qwen2.5 7B can route the role but a single unbounded catalog remains unsafe. Measured prompt growth is ~2.6k at 2 evidence rows, ~6.6k at 10, ~11.5k at 20 and ~16.4k at 30, while catalog projection can include up to 1000 rows. The next correction must define a dedicated governed binder contract plus bounded/batched evidence projection and output cardinality, not just increase budget or alias the name.
