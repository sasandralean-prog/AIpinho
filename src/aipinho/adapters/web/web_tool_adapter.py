...
class WebToolAdapter:
    def describe(self, tool: ToolDefinition) -> dict[str, object]:
        return {"adapter": "web", "tool_id": tool.tool_id, "dry_run_only": True}
    ...
    def dry_run(self, tool: ToolDefinition, call: ToolCall, safety: ToolSafetyDecision) -> ToolDryRunResultItem:
        url = str(call.input.get("url") or "<url unavailable>")
        return ToolDryRunResultItem(
            tool_id=tool.tool_id,
            status="needs_approval" if safety.approval_required_for else "simulated",
            would_do=f"Would request URL '{url}' in a future approved runtime; not executed and no network was used.",
    ...
    def execute(self, *args, **kwargs):
        raise NotImplementedError("real_web_execution_disabled_in_sprint06")
