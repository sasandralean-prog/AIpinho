...
class AndroidToolAdapter:
    def describe(self, tool: ToolDefinition) -> dict[str, object]:
        return {"adapter": "android", "tool_id": tool.tool_id, "dry_run_only": True}
    ...
    def dry_run(self, tool: ToolDefinition, call: ToolCall, safety: ToolSafetyDecision) -> ToolDryRunResultItem:
        return ToolDryRunResultItem(
            ...
            would_do=f"Would simulate Android tool action '{tool.action}'; not executed and adb/gradle were not called.",
    ...
    def execute(self, *args, **kwargs):
        raise NotImplementedError("real_android_execution_disabled_in_sprint06")
