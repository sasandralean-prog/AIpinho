...
class SupervisedExecutionLoop:
    ...
    def run(self, run_id):
        ...
        for index,step in enumerate(run.plan.steps):
            ...
            if step.required:
                context.limitations.extend(outcome.limitations); context.blocked_items.extend(outcome.blocked_items)
            else:
                context.warnings.extend([*outcome.limitations, *outcome.blocked_items])
