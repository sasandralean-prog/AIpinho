...
class CanonicalPublicChatService:
    ...
    def _persist_executable_preview(
        ...
        intent_map = {
            ...
            "intent_type": snapshot.intent.intent_type,
            "operation_type": snapshot.intent.operation_type,
            "requires_task": snapshot.intent.requires_task,
            "side_effect_requested": snapshot.intent.side_effect_requested,
            "readonly": snapshot.intent.readonly,
            "negative_constraints": dict(snapshot.intent.negative_constraints),
            "confidence": snapshot.intent.confidence,
            "evidence": list(snapshot.intent.evidence),
            "semantic_intent_graph": snapshot.intent.semantic_intent_graph.model_dump(mode="json"),
            "canonical_lifecycle_id": snapshot.lifecycle_id,
            "canonical_operation_id": snapshot.operation_contract.operation_id,
            "risk": snapshot.operation_contract.risk_level,
            "target_path": metadata["primary_target_path"],
            "target_paths": metadata["target_paths"],
            "workspace": {
                "path": metadata["workspace_path"],
                "declared": bool(metadata["workspace_path"]),
            },
            "requested_actions": list(metadata["requested_actions"]),
